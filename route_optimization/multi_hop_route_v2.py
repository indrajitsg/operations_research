...
def fetch_edges(db_path, dmin, dmax):
    ...
    For each (depicao, arricao), keep the row with the MIN distance in range.
    If multiple rows tie on distance, keep the one with the lexicographically
    smallest flightnum. Returns list of arcs (u, v, dist, flightnum).
    """
    import math
    ...
    q = """
        SELECT TRIM(UPPER(depicao)) AS dep,
               ...
               CAST(distance AS REAL) AS dist,
               TRIM(COALESCE(flightnum, '')) AS flightnum
        FROM schedules
    ...
    # Deduplicate by (dep, arr), keep min distance; break ties by flightnum
    best = {}  # (dep,arr) -> (dist, flightnum)
    for dep, arr, dist, flightnum in rows:
        key = (dep, arr)
        d = float(dist)
        fn = flightnum or ''
        if key not in best:
            ...
            bd, bfn = best[key]
            if d < bd or (math.isclose(d, bd) and fn < bfn):
                best[key] = (d, fn)

    edges = [(dep, arr, d, fn) for (dep, arr), (d, fn) in best.items()]
    return edges


def build_graph(edges):
    """
    edges: list of (u, v, w, flightnum)
    Returns:
      - nodes, arcs, cost, OUT, IN, flightnum_map
    """
    nodes = set()
    arcs = []
    cost = {}
    ...
    for u, v, w, fn in edges:
        nodes.add(u); nodes.add(v)
        arcs.append((u, v))
        cost[(u, v)] = float(w)
        flightnum_map[(u, v)] = fn
        OUT[u].append((u, v))
        IN[v].append((u, v))
    ...
    for n in list(nodes):
        OUT.setdefault(n, [])
        IN.setdefault(n, [])
    return nodes, arcs, cost, OUT, IN, flightnum_map


def prune_to_reachable(nodes, arcs, cost, OUT, IN, flightnum_map, src, dst):
    """
    Reduce problem size: keep only nodes/edges that are on some path from src to dst.
    Also prunes the flightnum_map accordingly.
    """
    # Forward from src
    f_reach = set()
    stack = [src]
    ...
    flightnum2 = {a: flightnum_map.get(a, '') for a in arcs2}
    ...
    return keep_nodes, arcs2, cost2, OUT2, IN2, flightnum2
...
def format_route(route_nodes, total_nm, flightnum=None):
    legs = []
    fn = flightnum or {}
    for i in range(len(route_nodes) - 1):
        u, v = route_nodes[i], route_nodes[i+1]
        tag = f" —[{fn.get((u, v))}]→ " if fn.get((u, v)) else " → "
        legs.append(f"{u}{tag}{v}")
    route_str = "  ".join(legs)
...
def main():
    ...
    nodes, arcs, cost, OUT, IN, flightnum = build_graph(edges)
    ...
    nodes2, arcs2, cost2, OUT2, IN2, flightnum2 = prune_to_reachable(
        nodes, arcs, cost, OUT, IN, flightnum, src, dst
    )
    ...
    route_str, total_nm = format_route(route, total, flightnum=flightnum2)
    ...
    for i in range(len(route) - 1):
        ...
        fn = flightnum2.get((u, v))
        fn_txt = f"  [{fn}]" if fn else ""
        print(f"  {u} -> {v} : {cost2[(u, v)]:.1f} nm{fn_txt}")
